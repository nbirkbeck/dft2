...
struct Timer {
  void start() { start_ = ((double)clock()) / CLOCKS_PER_SEC; }
  double stop() {
...
struct dft_funs {
  ...
  typedef void (*dft_2d_t)(const InputType* input,
                           std::complex<InputType>* output, int n,
                           dft_1d_t tform);
};
...
void validate_1d(int n, void (*func)(const InputType*, InputType*, int),
  ...
  for (int i = 0; i < n; ++i) {
    ...
    for (int k = 0; k <= n / 2; k++) {
      act[k] = std::complex<InputType>(
          compact[k], k > 0 && k < n / 2 ? compact[n / 2 + k] : 0);
    }
...
template <>
void transpose4x4_simd(const double* A, double* B, const int lda,
                       const int ldb) {
  ...
  _mm256_store_si256((__m256i*)&B[0 * ldb],
                     _mm256_permute2x128_si256(tmp0, tmp2, 0x20));
  _mm256_store_si256((__m256i*)&B[1 * ldb],
                     _mm256_permute2x128_si256(tmp1, tmp3, 0x20));
  _mm256_store_si256((__m256i*)&B[2 * ldb],
                     _mm256_permute2x128_si256(tmp0, tmp2, 0x31));
  _mm256_store_si256((__m256i*)&B[3 * ldb],
                     _mm256_permute2x128_si256(tmp1, tmp3, 0x31));
}
...
template <typename T>
inline void unpack_2d_output(const T* col_fft, std::complex<T>* output, int n) {
  for (int y = 0; y <= n / 2; ++y) {
    const int y2 = y + n / 2;
...
template <>
void unpack_2d_output(const float* packed, std::complex<float>* output, int n) {
  const int n2 = n / 2;
  output[0] = packed[0];
  ...
  for (int c = 1; c < n2; ++c) {
    output[0 * n + c] = std::complex<float>(packed[c], packed[c + n2]);
    output[n2 * n + c] =
        std::complex<float>(packed[n2 * n + c], packed[n2 * n + c + n2]);
  }
  for (int r = 1; r < n2; ++r) {
    output[r * n + 0] =
        std::complex<float>(packed[r * n], packed[(r + n2) * n]);
    output[r * n + n2] =
        std::complex<float>(packed[r * n + n2], packed[(r + n2) * n + n2]);
    ...
    for (int c = 4; c < n2; c += 4) {
      ...
      real1 = real1 - real2;
      imag1 = imag1 + imag2;
      _mm_store_ps((float*)(output + r * n + c), _mm_unpacklo_ps(real1, imag1));
    ...
    int r3 = n - r2;
    output[r2 * n + 0] =
        std::complex<float>(packed[r3 * n], -packed[(r3 + n2) * n]);
    output[r2 * n + n2] =
        std::complex<float>(packed[r3 * n + n2], -packed[(r3 + n2) * n + n2]);
...
template <>
void unpack_2d_output(const double* packed, std::complex<double>* output,
                      int n) {
  const int n2 = n / 2;
  output[0] = packed[0];
  output[n2 * n] = packed[n2 * n];
  ...
  for (int c = 1; c < n2; ++c) {
    output[0 * n + c] = std::complex<double>(packed[c], packed[c + n2]);
    output[n2 * n + c] =
        std::complex<double>(packed[n2 * n + c], packed[n2 * n + c + n2]);
  }
  for (int r = 1; r < n2; ++r) {
    output[r * n + 0] =
        std::complex<double>(packed[r * n], packed[(r + n2) * n]);
    output[r * n + n2] =
        std::complex<double>(packed[r * n + n2], packed[(r + n2) * n + n2]);
    ...
    int r3 = n - r2;
    output[r2 * n + 0] =
        std::complex<double>(packed[r3 * n], -packed[(r3 + n2) * n]);
    output[r2 * n + n2] =
        std::complex<double>(packed[r3 * n + n2], -packed[(r3 + n2) * n + n2]);
    for (int c = 1; c < std::min(4, n2); ++c) {
...
void dft_2d_simd2(const InputType* input, std::complex<InputType>* output,
                  int n, typename dft_funs<InputType>::dft_1d_t tform) {
...
void dft_2d_simd(const float* input, std::complex<float>* output, int n,
  ...
  for (int c = 1; c < 4; ++c) {
    output[0 * n + c] =
        std::complex<float>(out_real2_8x8[c], out_real2_8x8[c + 4]);
    output[4 * n + c] = std::complex<float>(out_real2_8x8[4 * n + c],
                                            out_real2_8x8[4 * n + c + 4]);
  }
  for (int r = 1; r < 4; ++r) {
    output[r * n + 0] =
        std::complex<float>(out_real2_8x8[r * n], out_real2_8x8[(r + 4) * n]);
    output[r * n + 4] = std::complex<float>(out_real2_8x8[r * n + 4],
                                            out_real2_8x8[(r + 4) * n + 4]);

    for (int c = 1; c < 4; ++c) {
      output[r * n + c] = std::complex<float>(
          out_real2_8x8[r * n + c] - out_real2_8x8[(r + 4) * n + c + 4],
          out_real2_8x8[(r + 4) * n + c] + out_real2_8x8[r * n + c + 4]);
    }

    int r2 = r + 4;
    int r3 = n - r2;
    output[r2 * n + 0] = std::complex<float>(out_real2_8x8[r3 * n],
                                             -out_real2_8x8[(r3 + 4) * n]);
    output[r2 * n + 4] = std::complex<float>(out_real2_8x8[r3 * n + 4],
                                             -out_real2_8x8[(r3 + 4) * n + 4]);
    for (int c = 1; c < 4; ++c) {
      output[r2 * n + c] = std::complex<float>(
          out_real2_8x8[r3 * n + c] + out_real2_8x8[(r3 + 4) * n + c + 4],
          -out_real2_8x8[(r3 + 4) * n + c] + out_real2_8x8[r3 * n + c + 4]);
    }
  }
  return;
}
...
template <int vec_size = 4, typename InputType = float>
void idft_2d_simd2(const std::complex<InputType>* input, InputType* output,
                   int n, typename dft_funs<InputType>::dft_1d_t forward,
...
void benchmark() {
  ...
  for (int i = 0; i < 1000000; ++i) {
    for (int j = 0; j < n; ++j) {
      ...
      // dft(&d[0], &act[0], n);
      // d[j % n] = 0;
      sum += out[0] + out[1] + out[2] + out[3] + out[4] + out[5] + out[6] +
             out[7];  // act[0].imag() + out[0];
    }
  }
  printf("sum: %f\n", sum);
}
...
template <typename InputType = float>
void validate_2d(int n, typename dft_funs<InputType>::dft_2d_t fun2d,
      ...
      for (int j = 0; j <= n / 2; ++j) {
        ...
        if (real_diff > kThreshold || imag_diff > kThreshold) {
          printf(
              "r[%d,%d] %f %f %s\n", i, j, act[n * i + j].real(),
              fft[w * i + j][0],
              std::abs(act[n * i + j].real() - fft[w * i + j][0]) < kThreshold
                  ? "ok"
                  : "bad");
          printf(
              "i[%d,%d] %f %f %s\n", i, j, act[n * i + j].imag(),
              fft[w * i + j][1],
              std::abs(act[n * i + j].imag() - fft[w * i + j][1]) < kThreshold
                  ? "ok"
                  : "bad");
          printf("\n");
          num_different++;
...
template <typename InputType = float>
void benchmark_2d(const int n, typename dft_funs<InputType>::dft_2d_t fun2d,
                  typename dft_funs<InputType>::dft_1d_t func,
  ...
  if (inv) {
    ...
    if (show_data) {
      for (int i = 0; i < n; ++i) {
        for (int j = 0; j < n; ++j) {
          printf("%f+%fi ", fft[n * i + j][0], fft[n * i + j][1]);
        }
        printf("\n");
      }
      ...
      for (int i = 0; i < n; ++i) {
        for (int j = 0; j <= n / 2; ++j) {
          printf("r[%d,%d] %f %f %s\n", i, j, act[n * i + j].real(),
                 fft[w * i + j][0],
                 std::abs(act[n * i + j].real() - fft[w * i + j][0]) < 1e-4
                     ? "ok"
                     : "bad");
          printf("i[%d,%d] %f %f %s\n", i, j, act[n * i + j].imag(),
                 fft[w * i + j][1],
                 std::abs(act[n * i + j].imag() - fft[w * i + j][1]) < 1e-4
                     ? "ok"
                     : "bad");
        }
        printf("\n");
  ...
  fprintf(stderr, "%d dft: %f  fftw: %f  speedup: %f  (%d)\n", n, dft_time,
          fftw_time, dft_time / fftw_time, sum > 1000);
}
...
int main(int ac, char* av[]) {
  ...
  if (av[1][0] == 'v') {
    ...
    // validate_2d<double>(2, dft_2d_simd2<2, double>, dft_2_compact<__m128d,
    // double>); validate_2d<double>(4, dft_2d_simd2<2, double>,
    // dft_4_compact<__m128d, double>);
    validate_2d<double>(8, dft_2d_simd2<2, double>,
                        dft_8_compact<__m128d, double>);
    validate_2d<double>(16, dft_2d_simd2<2, double>,
                        dft_16_compact<__m128d, double>);
    validate_2d<double>(32, dft_2d_simd2<2, double>,
                        dft_32_compact<__m128d, double>);
    validate_2d<double>(64, dft_2d_simd2<2, double>,
                        dft_64_compact<__m128d, double>);

    fprintf(stderr, "\nTesting simd2(256)[double]\n");
    // validate_2d<double>(4, dft_2d_simd2<4, double>, dft_8_compact<__m256d,
    // double>);
    validate_2d<double>(8, dft_2d_simd2<4, double>,
                        dft_8_compact<__m256d, double>);
    validate_2d<double>(16, dft_2d_simd2<4, double>,
                        dft_16_compact<__m256d, double>);
    validate_2d<double>(32, dft_2d_simd2<4, double>,
                        dft_32_compact<__m256d, double>);
    validate_2d<double>(64, dft_2d_simd2<4, double>,
                        dft_64_compact<__m256d, double>);
  }
  if (av[1][0] == '2') {
    fprintf(stderr, "\nTesting dft\n");
    benchmark_2d<float>(4, dft_2d<float>, dft_4_compact<float, float>,
                        idft_4_compact<float, float>);
    benchmark_2d<float>(8, dft_2d<float>, dft_8_compact<float, float>,
                        idft_8_compact<float, float>);
    benchmark_2d(16, dft_2d<float>, dft_16_compact<float, float>,
                 idft_16_compact<float, float>);
    benchmark_2d(32, dft_2d<float>, dft_32_compact<float, float>,
                 idft_32_compact<float, float>);
    benchmark_2d(64, dft_2d<float>, dft_64_compact<float, float>,
                 idft_64_compact<float, float>);

    fprintf(stderr, "\nTesting dft(double)\n");
    benchmark_2d<double>(4, dft_2d<double>, dft_4_compact<double, double>,
                         idft_4_compact<double, double>);
    benchmark_2d<double>(8, dft_2d<double>, dft_8_compact<double, double>,
                         idft_8_compact<double, double>);
    benchmark_2d<double>(16, dft_2d<double>, dft_16_compact<double, double>,
                         idft_16_compact<double, double>);
    benchmark_2d<double>(32, dft_2d<double>, dft_32_compact<double, double>,
                         idft_32_compact<double, double>);
    benchmark_2d<double>(64, dft_2d<double>, dft_64_compact<double, double>,
                         idft_64_compact<double, double>);
    ...
    fprintf(stderr, "\nTesting simd2(256)[double]\n");
    benchmark_2d<double>(8, dft_2d_simd2<4, double>,
                         dft_8_compact<__m256d, double>);
    benchmark_2d<double>(16, dft_2d_simd2<4, double>,
                         dft_16_compact<__m256d, double>);
    benchmark_2d<double>(32, dft_2d_simd2<4, double>,
                         dft_32_compact<__m256d, double>);
    benchmark_2d<double>(64, dft_2d_simd2<4, double>,
                         dft_64_compact<__m256d, double>);
    ...
    fprintf(stderr, "\nTesting simd2(128)[double]\n");
    benchmark_2d<double>(8, dft_2d_simd2<2, double>,
                         dft_8_compact<__m128d, double>);
    benchmark_2d<double>(16, dft_2d_simd2<2, double>,
                         dft_16_compact<__m128d, double>);
    benchmark_2d<double>(32, dft_2d_simd2<2, double>,
                         dft_32_compact<__m128d, double>);
    benchmark_2d<double>(64, dft_2d_simd2<2, double>,
                         dft_64_compact<__m128d, double>);
  }
  return 0;
}
